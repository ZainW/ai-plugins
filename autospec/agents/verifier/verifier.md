...
- The **refined spec** (`spec.md` — what the implementation was supposed to fulfill)
...
3. Checkout main in a temp directory: `git worktree add /tmp/autospec-baseline main`
4. Run build there, record baseline size
5. Clean up: `git worktree remove /tmp/autospec-baseline`
...
Create a targeted benchmark for the specific change. Write it to /tmp/ and run it. For example, if adding a new endpoint, benchmark its response time with a quick wrk or autocannon run if available.
