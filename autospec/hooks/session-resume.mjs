 ...
 * Session Resume Hook
 *
 * Fires on SessionStart to detect in-progress autospec sessions
...
async function main() {
  ...
  // ── 2. Check for .autospec/session.md ──────────────────────────────
  const stateDir = join(cwd, ".autospec");
  ...
  // ── 5. Detect orphaned autospec worktrees ──────────────────────────
  let worktreeInfo = [];
  try {
    ...
    for (const block of blocks) {
      ...
      const branch = lines
        ...
        ?.replace("branch ", "");
      if (branch && branch.includes("autospec")) {
  ...
  // ── 6. Reconcile state: check autospec/* branches ──────────────────
  let branches = [];
  try {
    const raw = execFileSync(
      "git",
      ["branch", "--list", "autospec/*", "--format=%(refname:short)"],
  ...
  if (implementationLogged && branches.length === 0) {
    reconciliation =
      "WARNING: Log shows implementation phase but no autospec/* branches found — state may be stale.";
